<?php
/*
  Plugin Name: WooCommerce Products Filter Light
...
final class WOOF
{
    ...
    public function init()
    {
        ...
        add_filter('posts_where', array($this, 'woof_post_title_filter'));
    ...
    public function wp_head()
    {
        ...
        <?php if (isset($this->settings['custom_css_code'])): ?>
            <style type="text/css">
            <?php echo stripcslashes($this->settings['custom_css_code']); ?>
...
//+++
    ...
    public function get_options()
    {
        $options = array
            ...
            array(
                'name' => __('Try to ajaxify the shop. Just emulation to check layout before purchase premium version!', 'woocommerce-products-filter'),
                'desc' => __('Select "Yes" if you want to TRY make filtering in your shop by AJAX. Not compatible for 100% of all wp themes. In premium version ONLY! Just for test shop page layout!', 'woocommerce-products-filter'),
                'id' => 'woof_try_ajax',
                'type' => 'select',
                'class' => 'chosen_select',
                'css' => 'min-width:300px;',
            ...
            array(
                ...
                'desc' => __('Show textinput for searching by products title. In premium version ONLY!', 'woocommerce-products-filter'),
            ...
            array(
                ...
                'desc' => __('Show woocommerce filter by price inside woof search form. In premium version ONLY!', 'woocommerce-products-filter'),
    ...
    public function woof_shortcode($atts)
    {
        ...
        if (!empty($taxonomies))
        {
            foreach ($taxonomies as $tax_key => $tax)
            {
                ...
                $hide_empty = (bool) get_option('woof_hide_dynamic_empty_pos');
    ...
    public function woof_exclude_tax_key($terms)
    {
        if (!defined('DOING_AJAX'))
        {
            if (isset($_SESSION['woof_really_current_term']))
            {
                ...
                $queried_obj = $_SESSION['woof_really_current_term'];
                $current_term_id = $queried_obj->term_id;
                ...
                {
                    //lets look top level items first
                    if (!empty($terms) AND is_array($terms))
                    {
                        foreach ($terms as $tid => $value)
                        {
                            if ($tid == $parent_id)
                            {
                                if (isset($terms[$tid][$current_term_id]['childs']))
                                {
                                    $terms = $terms[$tid][$current_term_id]['childs'];
                                }
                                break;
                            }
                        }
                        ...
                        if (empty($terms))
                        {
                            //if we not found parent, look into each array recursively
                            foreach ($terms as $tid => $value)
                            {
                                if (!empty($value['childs']))
                                {
                                    foreach ($value['childs'] as $tid2 => $v)
                                    {
                                        if (isset($terms[$tid][$tid2][$current_term_id]['childs']))
                                        {
                                            $terms = $terms[$tid][$tid2][$current_term_id]['childs'];
                                            break(2);
                                        } else
                                        {
                                            $terms = $this->_woof_exclude_tax_key_util1($current_term_id, $v['childs']);
                                        }
                                    }
                                }
                            }
    ...
    private function _woof_exclude_tax_key_util1($current_term_id, $child_terms)
    {
        $terms = array();
        if (!empty($child_terms))
        {
            foreach ($child_terms as $tid => $v)
            {
                if (isset($terms[$tid][$current_term_id]['childs']))
                {
                    $terms = $terms[$tid][$current_term_id]['childs'];
